...
kappa0 <- 0.5; a0 <- 2; b0 <- sigma0_sq * (a0 - 1)
...
# At each step: discount old stats by rho, then absorb new obs.
# Effective sample size saturates at 1/(1-rho).
# -----------------------------

update_nig_discount <- function(y_new, m_prev, k_prev, a_prev, b_prev, rho) {
  k_disc <- rho * k_prev          # forget old pseudo-counts
  kn     <- k_disc + 1            # absorb one new obs
...
K   <- 10
rho <- 0.9   # effective sample size ≈ 1/(1-rho) = 10 — comparabile con K
...
# stato ricorsivo per discount (inizia dal prior)
...
for (i in 2:n_true) {
  ...
  # Discounted — aggiorna ricorsivamente con l'osservazione i-1
...
methods <- list(
  ...
  list(med = dis_med, p10 = dis_p10, p90 = dis_p90,
       ...
       title = paste0("Discounted (ρ=", rho, ", n_eff≈", round(1/(1-rho)), ")"))
)
